const articles = [
  {
    name: "Komiks La Loba",
    nameEn: "La Loba comic",
    image: "/IMG/22/1.jpg",
    description:
      "La Loba doslova znamená ona – vlk, legenda vzniklá v Pueblu v severním Mexiku. Pro mě je to mýtus o přirozené divokosti a procesu tvorby života, témata úzce spjata s ženstvím. La Loba sbírá vlčí kosti a magickým rituálem je přivádí opět k životu, ovšem transformace nekončí pouze u vlčí podoby, postupně se proměňuje vlk ve smějící se ženu. Komiks je možné zakoupit v obchodě Page Five nebo ho objednat na mém mailu fischmeister.art@gmail.com",
    descriptionEn:
      "With this comic I wanted to bring to life an old legend La Loba about wild women resurrected from wolf bones. La Loba literally means she-wolf. It’s the name of a mythical woman of the Pueblo people from Northern Mexico, popularized by Clarissa Pinkola Estés in her book Women Who Run With The Wolves. La Loba to me means fierceness and the proces of creation which are very important aspects of femininity. You can buy the comic at Page Five store or order directly at fischmeister.art@gmail.com",
    resources: [
      {
        image: "/IMG/22/1.jpg",
      },
      {
        image: "/IMG/22/2.jpg",
      },
      {
        image: "/IMG/22/3.jpg",
      },
      {
        image: "/IMG/22/4.jpg",
      },
      {
        image: "/IMG/22/5.jpg",
      },
      {
        image: "/IMG/22/6.jpg",
      },
    ],
  },
  {
    name: "Ilustrace pro noviny Právo",
    nameEn: "Illustration for newspaper Právo",
    image: "/IMG/21/1.jpg",
    description: "",
    descriptionEn: "",
    resources: [
      {
        image: "/IMG/21/1.jpg",
      },
    ],
  },
  {
    name: "Etiketa pro pivovar Zichovec",
    nameEn: "Beer label for Zichovec",
    image: "/IMG/20/1.jpg",
    description: "",
    descriptionEn: "",
    resources: [
      {
        image: "/IMG/20/1.jpg",
      },
    ],
  },
  {
    name: "Květiny",
    nameEn: "Flowers",
    image: "/IMG/19/1.jpg",
    description: "osobní projekt",
    descriptionEn: "personal project",
    resources: [
      {
        image: "/IMG/19/1.jpg",
      },
      {
        image: "/IMG/19/2.jpg",
      },
      {
        image: "/IMG/19/2.jpg",
      },
    ],
  },
  {
    name: "Peace of mind",
    nameEn: "Peace of mind",
    image: "/IMG/18/1.jpg",
    description: "osobní projekt",
    descriptionEn: "personal project",
    resources: [
      {
        image: "/IMG/18/1.jpg",
      },
      {
        image: "/IMG/18/2.jpg",
      },
    ],
  },
  {
    name: "Plakát pro Hong Kong Alliance",
    nameEn: "Poster for Hong Kong Alliance",
    image: "/IMG/17/1.jpg",
    description: "",
    descriptionEn: "",
    resources: [
      {
        image: "/IMG/17/1.jpg",
      },
    ],
  },
  {
    name: "Ilustrace do publikace Suterén, Artrafika",
    nameEn: "Illustration for Artrafika",
    image: "/IMG/16/1.jpg",
    description: "",
    descriptionEn: "",
    resources: [
      {
        image: "/IMG/16/1.jpg",
      },
    ],
  },
  {
    name: "Ilustrace pro noviny Právo",
    nameEn: "Ilustrace pro noviny Právo",
    image: "/IMG/15/1.jpg",
    description: "povídka Aleny Mornštajnové,  pro noviny Právo",
    descriptionEn: "Illustration for newspaper Právo",
    resources: [
      {
        image: "/IMG/15/1.jpg",
      },
    ],
  },
  {
    name: "Benátky",
    nameEn: "Venice",
    image: "/IMG/14/1.jpg",
    description: "kalendář na téma benátského karnevalu, GRAFIE s.r.o.",
    descriptionEn: "calendar on themes of the Venice carnival, GRAFIE s.r.o.",
    resources: [
      {
        image: "/IMG/14/1.jpg",
      },
    ],
  },
  {
    name: "Dům",
    nameEn: "House",
    image: "/IMG/13/1.jpg",
    description: "povídka Jiřího Kratochvíla, pro noviny Právo",
    descriptionEn: "Illustration for newspaper Právo",
    resources: [
      {
        image: "/IMG/13/1.jpg",
      },
    ],
  },
  {
    name: "Plakát ke 30. výročí Sametové revoluce",
    nameEn: "Poster for 30th anniversary of Velvet revolution",
    image: "/IMG/12/1.jpg",
    description: "",
    descriptionEn: "",
    resources: [
      {
        image: "/IMG/12/1.jpg",
      },
    ],
  },
  {
    name: "Obálka a ilustrace ke knize Revoluce od Jakuba Trpiše",
    nameEn: "Cover and illustration for book Revolution by Jakub Trpiš",
    image: "/IMG/11/1.jpg",
    description: "",
    descriptionEn: "",
    resources: [
      {
        image: "/IMG/11/1.jpg",
      },
      {
        image: "/IMG/11/2.jpg",
      },
      {
        image: "/IMG/11/3.jpg",
      },
      {
        image: "/IMG/11/4.jpg",
      },
      {
        image: "/IMG/11/5.jpg",
      },
      {
        image: "/IMG/11/6.jpg",
      },
      {
        image: "/IMG/11/7.jpg",
      },
      {
        image: "/IMG/11/8.jpg",
      },
      {
        image: "/IMG/11/9.jpg",
      },
      {
        image: "/IMG/11/10.jpg",
      },
    ],
  },
  {
    name: "Portrét mého otce",
    nameEn: "Portrait of my father",
    image: "/IMG/10/1.jpg",
    description: "",
    descriptionEn: "",
    resources: [
      {
        image: "/IMG/10/1.jpg",
      },
    ],
  },
  {
    name: "O neposlušných kůzlátkách",
    nameEn: "O neposlušných kůzlátkách",
    image: "/IMG/9/1.jpg",
    description: "povídka Biancy Bellové, pro noviny Právo",
    descriptionEn: "Illustration for newspaper Právo",
    resources: [
      {
        image: "/IMG/9/1.jpg",
      },
    ],
  },
  {
    name: "Co se skrývá v domech",
    nameEn: "What hides in homes",
    image: "/IMG/8/1.jpg",
    description: "osobní projekt",
    descriptionEn: "personal project",
    resources: [
      {
        image: "/IMG/8/1.jpg",
      },
      {
        image: "/IMG/8/2.jpg",
      },
      {
        image: "/IMG/8/3.jpg",
      },
    ],
  },
  {
    name: "Oslava 50. výročí přistání na měsíci",
    nameEn: "Celebration of 50th anniversary of the moon landing",
    image: "/IMG/7/1.jpg",
    description: "",
    descriptionEn: "",
    resources: [
      {
        image: "/IMG/7/1.jpg",
      },
    ],
  },
  {
    name: "Obálka a ilustrace ke knize Divočina",
    nameEn: "Cover and illustrations for book Divočina",
    image: "/IMG/6/1.jpg",
    description: "Povídky pro DUHU, vydala Novela Bohemica",
    descriptionEn:
      "Povídky pro DUHU (Wilderness: Stories for DUHA), published by Novela Bohemica",
    resources: [
      {
        image: "/IMG/6/1.jpg",
      },
      {
        image: "/IMG/6/2.jpg",
      },
      {
        image: "/IMG/6/3.jpg",
      },
      {
        image: "/IMG/6/4.jpg",
      },
      {
        image: "/IMG/6/5.jpg",
      },
      {
        image: "/IMG/6/6.jpg",
      },
      {
        image: "/IMG/6/7.jpg",
      },
      {
        image: "/IMG/6/8.jpg",
      },
    ],
  },
  {
    name: "Amazonské pralesy",
    nameEn: "Amazon rainforest",
    image: "/IMG/5/1.jpg",
    description: "osobní projekt",
    descriptionEn: "personal project",
    resources: [
      {
        image: "/IMG/5/1.jpg",
      },
    ],
  },
  {
    name: "Notre Dame",
    nameEn: "Notre Dame",
    image: "/IMG/4/1.jpg",
    description: "osobní projekt",
    descriptionEn: "personal project",
    resources: [
      {
        image: "/IMG/4/1.jpg",
      },
    ],
  },
  {
    name: "Hong Kong",
    nameEn: "Hong Kong",
    image: "/IMG/3/1.jpg",
    description: "osobní projekt",
    descriptionEn: "personal project",
    resources: [
      {
        image: "/IMG/3/1.jpg",
      },
    ],
  },
  {
    name: "",
    nameEn: "",
    image: "/IMG/2/1.jpg",
    description: "osobní projekt",
    descriptionEn: "personal project",
    resources: [
      {
        image: "/IMG/2/1.jpg",
      },
    ],
  },
  {
    name: "Dream week",
    nameEn: "Dream week",
    image: "/IMG/1/1.jpg",
    description: "osobní projekt",
    descriptionEn: "personal project",
    resources: [
      {
        image: "/IMG/1/1.jpg",
      },
      {
        image: "/IMG/1/2.jpg",
      },
      {
        image: "/IMG/1/3.jpg",
      },
      {
        image: "/IMG/1/4.jpg",
      },
      {
        image: "/IMG/1/5.jpg",
      },
      {
        image: "/IMG/1/6.jpg",
      },
      {
        image: "/IMG/1/7.jpg",
      },
    ],
  },
];

export default articles;
